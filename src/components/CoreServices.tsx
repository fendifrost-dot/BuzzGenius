...
const CoreServices = () => (
  <section className="bg-matte-dark text-primary-foreground">
    <div className="container mx-auto px-6 py-20 md:py-28 max-w-4xl">
      <h2 className="text-2xl md:text-3xl font-bold text-primary-foreground mb-8">Core Services</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-6">
        {services.map((service) => (
          <div key={service} className="flex items-start gap-3">
            ...
            <span className="text-chrome-light text-base md:text-lg font-medium">
              {service}
            </span>
          </div>
        ))}
      </div>
      <p className="mt-6 text-xs text-chrome italic">
