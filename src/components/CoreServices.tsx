    ...
    <div className="container mx-auto px-6 py-24 md:py-36 max-w-4xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-0">
        {services.map((service) => (
          <div key={service} className="group flex items-center gap-4 py-5 border-b border-chrome/10 transition-colors duration-300 hover:border-gold/40">
            <span className="block w-5 h-px bg-gold/60 group-hover:w-8 group-hover:bg-gold transition-all duration-300 flex-shrink-0" />
            <span className="text-chrome-light text-sm md:text-base font-light uppercase tracking-wide group-hover:text-primary-foreground transition-colors duration-300">{service}</span>
          </div>
        ))}
      </div>
